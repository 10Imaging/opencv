...
#$1 [build ABIs] array, e.g. "arm7-android x86_64-osx"
{
  ...
  case $i in
    arm7-android)
#    COMMON_OPTIONS="-DCMAKE_VERBOSE_MAKEFILE=ON -DENABLE_NEON=ON -DWITH_TBB=OFF -DBUILD_TBB=ON -DWITH_CUDA=OFF\
    COMMON_OPTIONS="-DENABLE_NEON=ON -DWITH_TBB=OFF -DBUILD_TBB=ON -DWITH_CUDA=OFF\
     ...
     -DANDROID_NATIVE_API_LEVEL=21 -DANDROID_SDK_TARGET=21 -DANDROID_STL=c++_shared\
     -DCMAKE_TOOLCHAIN_FILE=${ANDROID_NDK}/build/cmake/android.toolchain.cmake"
#     -DCMAKE_CXX_FLAGS_DEBUG=-fdebug-prefix-map=${HOME}=~\
#     -DCMAKE_C_FLAGS_DEBUG=-fdebug-prefix-map=${HOME}=~"
    EXTRA_OPTIONS="-DWITH_OPENCL=${ENABLE_OPENCL} -DANDROID_TOOLCHAIN_NAME=clang"
...
#     -DCMAKE_C_FLAGS_DEBUG=-fdebug-prefix-map=${HOME}=~"
    ...
    ;;
    x86_64-osx)
    COMMON_OPTIONS="-DWITH_TBB=OFF -DBUILD_TBB=ON -DWITH_CUDA=OFF\
